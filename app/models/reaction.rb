class Reaction < ApplicationRecord
    ...
    def create_reaction(products, reactants, catalysts)
        ...
        "#{reactants_str}>#{catalysts_str}>#{products_str}"
    end
    ...
    def create_reaction_dict(products, reactants, catalysts)
        {
          "reactants" => get_smiles_array(reactants),
          "products" => get_smiles_array(products),
          "catalysts" => get_smiles_array(catalysts)
        }
    end
    ...
    def concentration_formula(compound, reaction, power_number, type)
        numerators = {
                    'mass/vol': compound.mass,
                    'Molarity': mols_from_mass(compound.molecular_weight, compound.mass),
    ...
    def mols_from_mass(molecular_weight, mass_in_g) #should this be in compound not reaction?
        mols = mass_in_g / molecular_weight
        mols
    end

    def single_eq_mol_from_all_mols(reactants)  #finds single eq when mols or mass is known for every reactant
        reactant_mols = get_reactant_mols_hash(reactants)
        sorted_reactant_mols = reactant_mols.sort_by { |_, value| value }
        least_mols = sorted_reactant_mols.values.first
        least_mols
    end

    def single_eq_mol_from_mol_and_eq(reactant)  #use when eq and mass/mols are known --> gives mols for 1 eq of compound
        single_eq_mols = nil

        #   if reactant.mols && reactant.equivalents || reactant.mass_used && reactant.equivalents 
        if reactant.mols?
            mols = reactant.mols
        else 
            mols = mols_from_mass(reactant)
        end
    ...
    def find_single_eq_mol_from_unknown(reactants)   # --> use when determining conditions are unknown a) all mols known, b) known with mol + eq (apply to  unknown with eq to find mol)
        if check_reactants_mols_or_mass(reactants) == true
            single_eq_mols = single_eq_mol_from_all_mols(reactants)
        elsif check_for_mols_or_mass_and_eq(reactants)[:result] == true
            single_eq_mols = mol_eq_from_mol_and_eq(check_for_mols_or_mass_and_eq(reactants)[:reactant])
        end
        single_eq_mols
    end  
    
    def mols_from_neighbour(reactant, compound_of_interest)
        single_eq_mol = mol_eq_from_mol_and_eq(reactant) #reactant with mol and eq 
        mols = compound_of_interest.equivalents * single_eq_mol #compound with eq
        mols
    end

    def limiting_reactant(reactants)
        #calculate_all possible unknowns before doing this 
        limiting_reactants = []
        single_eq_mol = find_single_eq_mol_from_unknown(reactants)
        reactant.each do |reactant|
            if reactant.mols == single_eq_mol || reactant.equivalents == 1
                limiting_reactants << reactant
            end
        end
        limiting_reactants
    end
    ...
    def eq_from_all_mols(reactants, compound_of_interest) #all mols are known, previously eq_for_compound
        least_mols = single_eq_mol_from_all_mols(reactants)
        equivalents = fdiv(compound_of_interest.mols, least_mols)
        equivalents
    end

    def eq_from_neighbour(reactant, compound_of_interest)
        # if reactant.mols && reactant.equivalents || reactant.mass_used && reactant.equivalents 
        if reactant.mols?
            reactant_mols = reactant.mols
        else 
            reactant_mols = mols_from_mass(reactant)
        end

        if compound_of_interest.mols?
            compound_mols = compound_of_interest.mols
        else 
            compound_mols = mols_from_mass(compound_of_interest)
        end

        single_eq_mol =  single_eq_mol_from_mol_and_eq(reactant)
        compound_eq = fdiv(compound_mols, single_eq_mol)
        compound_eq
    end
    ...
    def mass_from_neighbour(reactant, compound_of_interest)
        single_eq_mol = mol_eq_from_mol_and_eq(reactant) #reactant with mol and eq 
        mols = compound_of_interest.equivalents * single_eq_mol #compound with eq
        molecular_weight = compound_of_interest.molecular_weight
    ...
    def find_max_yeild(product, reactants) #have to have stoicheometry 
        limiting reactant = 
        limiting_mols = find_single_eq_mol_from_unknown(reactant)
        #if product.composing_reactants.include?(limiting_reactant) && if !reactants.include?(product)
        molecular_weight = product.compounds.molecular_weight
        
        if product.equivalents? 
            product_eq = product.equivalents
        else
            product_eq = find_product_eq(product)
        end

        yeild_mols = fdiv(product_eq, limiting_reactant.equivalents) * limiting_mols
        yeild_mass = mass_from_mols(yeild_mols, molecular_weight)
    ...
    #def find_product_eq(product)
    # fill_table_using_knowns(reactants,products)
...
# 2. eq_from_all mass: calculate mols then apply #1 

# 3. eq_from_mixed: calculate mols for the ones with mass then apply #1 

# 4. eq_from_neighbour: (eq and mols/mass in known, (calculate mols if mass),  need mol/mass in unknown )

# Mol: 
# 1. mols_from_mass: need mass of same reactant 

# 2. mols_from_neighbour: need one known with eq and mol/mass and  eq of unknown

# Mass:

# 1. mass_from_mols: need mol of same reactant

# 2. mass_from_neighbour: need one known with eq and mol/mass and  eq of unknown
    ...
    def get_reactant_mols_hash(reactants)  # finds mols of reactants from if mass or mols is present
        ...
        reactants.each do |reactant|
            if reactant.mols?
                ...
                mols = reactant.mols_from_mass(reactant.compound.molecular_weight, reactant.mass)
            end
            reactant_mols[reactant] = mols
        end
        reactant_mols
...
#check_conditions_for_mols

#check_conditions_for_mass

#check_conditions_for_eq
